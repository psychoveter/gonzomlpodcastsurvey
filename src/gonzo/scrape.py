...
Usage:
    python -m gonzo.scrape --since 2024-05-11 --db data/gonzo.db
"""
...
def fetch(before: int | None, *, session: requests.Session, retries: int = 4) -> str:
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--resume", action="store_true",
                   help="continue from the smallest id already in DB")
    ...
    if args.resume:
        ...
        if row and row["m"] is not None:
            before = int(row["m"])
            print(f"[resume] starting before={before}", file=sys.stderr)
    ...
    while pages < args.max_pages:
        pages += 1
        html = fetch(before, session=session)
        records = parse_page(html)
        if not records:
            print(f"[stop] empty page (before={before})", file=sys.stderr)
            break
        ...
        with conn:
            for r in records:
                dbmod.insert_message(conn, r["msg"])
                dbmod.insert_links(conn, r["msg"]["id"], r["links"])
                new_in_page += 1
        total_new += new_in_page
        page_min = min_id(records)
        print(
            f"page {pages:>3}  before={before}  ids {page_min}..{max(r['msg']['id'] for r in records)}  "
            f"{newest.date()}..{oldest.date()}  new={new_in_page}  total={total_new}",
            file=sys.stderr,
        )
        if oldest < since:
            print(f"[stop] reached cutoff {since.date()} (oldest on page {oldest.date()})", file=sys.stderr)
            break
        if page_min is None:
            break
        if before is not None and page_min >= before:
            print(f"[stop] no progress: page_min {page_min} >= before {before}", file=sys.stderr)
            break
    ...
    print(f"done. pages={pages} total_new={total_new}", file=sys.stderr)
    return 0
